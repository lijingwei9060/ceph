...
inline ceph::libfdb::future_value await_future_of(std::invocable auto fn, auto&& ...params)
{
 future_value fv = fn(std::forward(params)...);
...
// JFW: this can be consolidated into something more general, but it will take some focus-- right now, "getting it to work" is
// the goal
